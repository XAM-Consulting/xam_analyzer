...
import 'package:xam_analyzer/src/analyzers/lint_analyzer/models/lint_file_report.dart';
...
class LintAnalysisOptionsValidator {
  ...
  static List<_RuleWithSpan>? _getRulesList(YamlNode node) {
    if (node is YamlMap) {
      final rules = (node['xam_analyzer'] as YamlMap?)?['rules'] as YamlNode?;
      ...
      if (!ids.contains(rule.ruleName)) {
        issues.add(
          Issue(
            ...
            documentation: Uri.parse('https://xam.dev/docs/individuals/rules'),
